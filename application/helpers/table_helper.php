...
function get_room_data_row($room,$controller)
{
    ...
    $table_data_row.='<td width="10%">'.to_currency($room->tempPrice).'</td>';
    $table_data_row.='<td width="20%">'.$room->temp_duration.'</td>';
    $table_data_row.='<td width="10%">'.to_currency($room->nightPrice).'</td>';
...
{
    $ITEM_NUMBER_IN_ROW = 5;
    $CI =& get_instance();
    $controller_name=strtolower(get_class($CI));
    
    $table='<table class="table_rooms"><tbody>';
    if($rooms->num_rows()==0)
    {
            ...
            if($item_number < $ITEM_NUMBER_IN_ROW)
            {
                ...
                {              
                    $table.=anchor($controller_name."/room_manage_view/$room->room_id/width:200/height:120", 
                                    get_rooms_booking_style($room, $controller),
                                    array('id'=>$room->name,
                                    'class'=>'thickbox',
                ...
                {
                   $table.=get_rooms_booking_style($room, $controller);
                }

                $table.= $CI->lang->line('rooms_booking_room_name').$room->name;
                $table.="<br>";
                $table.= $CI->lang->line('rooms_booking_temporaly_booking_price').to_currency($room->tempPrice);
                $table.="<br>";
                $table.= $CI->lang->line('rooms_booking_night_booking_price').to_currency($room->nightPrice);
                $table.="</td>";
                $item_number++;
            }
...
function get_rooms_booking_style($room, $controller)
{
    $CI =& get_instance();
    $controller_name=strtolower(get_class($CI));
    $booking_detail = $controller->Booking_detail->get_room_booking($room->room_id); 
    $room_style;
    if($room->status == 1)
    {
    ...
    {
        $room_style = "<div class='room_button room_button_disable'>
                        <div class='room_disable'>".$CI->lang->line($controller_name.'_not_available')."</div>";
    }    
...
function get_booking_report_data_row($booking,$controller)
{
    ...
    $table_data_row.='<td width="14%">'.to_currency($booking->booking_price).'</td>';
